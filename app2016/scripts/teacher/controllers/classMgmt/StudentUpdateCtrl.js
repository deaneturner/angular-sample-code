...
define([
    ...
    function () {
        return ['$rootScope', '$scope', 'config', 'studentSrv', '$timeout', '$translate', '$filter', 'waitScreenSrv',
            '$state', '$q',
            function ($rootScope, $scope, config, service, $timeout, $translate, $filter, waitScreenSrv, $state, $q) {
                ...
                resultSet = {
                    data: {},
                    load: function (id) {
                        console.log('StudentUpdateCtrl:getStudent(' + id + ')');
                    ...
                    update: function (data) {
                        console.log('StudentUpdateCtrl:updateStudent(' + JSON.stringify(data) + ')');
                        waitScreenSrv.toggleActivity(true, 'classMgmtStudentWrite-spinner');

                        return service.update(data).then();
                    }
                };
                ...
                displayMngr = {
                    ...
                    contentBar: {
                        ...
                        breadcrumbs: {
                            ...
                            config: [
                            //    {
                            //    title: $state.studentUpdate && $state.studentUpdate.params.className,
                            //    onClick: function () {
                            //        $state.go($state.classMgmt.state, $state.studentUpdate.params);
                            //    }
                            //},
                                {
                                title: $state.studentUpdate && $state.studentUpdate.params.studentName
                            }]
                        }
                    },
                    navigator: {
                        ...
                        showValidSection: function (formName) {
                            ...
                            if (Array.isArray(formName)) {
                                ...
                                angular.forEach(formName, function (form) {
                                    if ($scope.studentForm[form] && $scope.studentForm[form].$invalid) {
                                        result = true;
                                    }
                                });
                            }
                            else {
                                result = $scope.studentForm[formName] && $scope.studentForm[formName].$invalid;
                            }
                            return result;
                        }
                    },
                    goHome: function () {
                        alertMngr.ignoreEnabled = true;
                ...
                formMngr = {
                    ...
                    submit: function () {
                        ...
                        if ($scope.studentForm.$valid) {
                            resultSet.update(resultSet.data).then(function (response) {
                                displayMngr.goHome();
                            }, function (error) {
                                $translate('teacher.student.errors.updateStudent').then(function (msg) {
                                    config.logger.error(msg, error);
                                });
                                updateScreenStatus();
                            });
                ...
                alertMngr = {
                    ...
                    stickyState: false,
                    modalConfig: {
                        title: $filter('translate')('classMgmt.student.confirmModal.title'),
                        text: $filter('translate')('classMgmt.student.confirmModal.text'),
                        type: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#DD6B55',
                        confirmButtonText: $filter('translate')('classMgmt.student.confirmModal.confirmButtonText'),
                        closeOnConfirm: true
                    },
                    init: function () {
                        var preventDefault,
                        ...
                        onStateChangeStart = function (event, toState, toParams, fromState, fromParams, options) {
                            self.enabled = $scope.studentForm && $scope.studentForm.$dirty;
                            if (self.enabled && !self.ignoreEnabled && (!alertMngr.stickyState || fromParams.class !== toParams.class)) {
                                try {
                ...
                var init = function () {
                    ...
                    if ($state.studentUpdate && ($state.params.class !== $state.studentUpdate.params.class)) {
                        /*
